...
const MAX_WII_SECTORS: usize = 143432 * 2; // Dual Layer
const SINGLE_LAYER_WII_SECTORS: usize = 143432;
...
impl SplitWbfsWriter {
    ...
    fn get_file(&mut self, offset: u64) -> Result<(&mut File, u64), WbfsError> {
        let file_index = (offset / self.split_size) as usize;
    ...
    fn write(&mut self, mut offset: u64, mut data: &[u8]) -> Result<(), WbfsError> {
        ...
        while !data.is_empty() {
            ...
            let writable_len = (split_size - rel_offset) as usize;
    ...
    fn truncate(&mut self, total_size: u64) -> Result<(), WbfsError> {
        ...
        let mut keys: Vec<_> = self.file_handles.keys().cloned().collect();
        keys.sort();
        ...
        for i in keys {
            ...
            debug!("Truncating file index {} to {} bytes.", i, chunk_size);
...
impl WbfsConverter {
    ...
    fn aes_decrypt(&self, key: &[u8; 16], iv: &[u8; 16], data: &mut [u8]) -> Result<(), WbfsError> {
    ...
    fn decrypt_title_key(&self, ticket: &[u8]) -> Result<[u8; 16], WbfsError> {
        let mut iv = [0u8; 16];
    ...
    fn read_iso_partition_data(
        ...
        while size > 0 {
            ...
            let mut raw_block = self.read_iso_data(block_offset, WII_SECTOR_SIZE as usize)?;
            ...
            let read_len = std::cmp::min(size as usize, 0x7C00 - offset_in_block);
    ...
    fn traverse_fst(&mut self, part_offset: u64, fst_data: &[u8]) -> Result<(), WbfsError> {
        ...
        debug!("FST has {} entries.", num_entries);
    ...
    fn build_disc_usage_table(&mut self) -> Result<(), WbfsError> {
        ...
        (u32::from_be_bytes(part_table_info[4..8].try_into().unwrap()) as u64) * 4;

        debug!(
            "Found {} partitions at offset {:#x}",
            num_partitions, part_table_offset
        );
        let part_info_data =
        ...
        for i in 0..num_partitions {
            ...
            let part_offset =
            (u32::from_be_bytes(part_info_data[offset..offset + 4].try_into().unwrap()) as u64)
            * 4;
            let part_type =
            u32::from_be_bytes(part_info_data[offset + 4..offset + 8].try_into().unwrap());
            info!(
                "Analyzing Partition {}: type={}, offset={:#x}",
                i, part_type, part_offset
            );
            ...
            (u32::from_be_bytes(part_header[0x14..0x18].try_into().unwrap()) as u64) * 4;

            let part_main_header = self.read_iso_partition_data(part_offset, 0, 0x480)?;
            let fst_offset =
            (u32::from_be_bytes(part_main_header[0x424..0x428].try_into().unwrap()) as u64) * 4;
            let fst_size =
            (u32::from_be_bytes(part_main_header[0x428..0x42C].try_into().unwrap()) as u64) * 4;

            debug!(
                "FST located at offset {:#x} with size {:#x}",
                fst_offset, fst_size
            );
    ...
    pub fn convert(
        ...
        let total_blocks_to_process = if last_used_wii_sector < SINGLE_LAYER_WII_SECTORS {
            debug!("Single-layer disc detected. Processing {} blocks.", single_layer_wbfs_blocks);
            single_layer_wbfs_blocks
        } else {
            let dual_layer_wbfs_blocks = MAX_WII_SECTORS >> wbfs_block_size_shift;
            debug!("Dual-layer disc detected. Processing {} blocks.", dual_layer_wbfs_blocks);
        ...
        let mut disc_info = vec![0u8; (0x100 + (MAX_WII_SECTORS >> wbfs_block_size_shift) * 2) as usize];
        ...
        for i in 0..total_blocks_to_process {
            ...
            if is_used {
                ...
                let wbfs_block_data = self.read_iso_data(iso_offset, wbfs_sec_sz as usize)?;

                let wbfs_offset = (block_addr as u64) * wbfs_sec_sz;
        ...
        let mut wbfs_head = vec![0u8; hd_sector_size as usize];
        let n_hd_sec = ((free_block_allocator as u64) * wbfs_sec_sz) / hd_sector_size;
        wbfs_head[0..4].copy_from_slice(&WBFS_MAGIC.to_be_bytes());
        wbfs_head[4..8].copy_from_slice(&(n_hd_sec as u32).to_be_bytes());
        wbfs_head[8] = 9; // log2(512)
        wbfs_head[9] = wbfs_sec_sz_s as u8;
        wbfs_head[12] = 1; // Mark disc slot 0 as used
        writer.write(0, &wbfs_head)?;

        let final_size = (free_block_allocator as u64) * wbfs_sec_sz;
